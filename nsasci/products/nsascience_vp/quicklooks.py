...
def plot_avg(a, variable, direction = 'up', show_std = True, **kwargs):
    ...
    df_mean = df[direction]
    df_std = df[f'{direction}_std']
    # if direction == 'up':
    #     df_mean = df.up
    #     df_std = df.up_std
    # elif direction == 'down':
    ...
    a.axhline(df_mean, **kwargs)
    # print('not na?')
    # print(~np.isnan(df_std))
    if show_std and ~np.isnan(df_std):
        a.axhspan(df_mean - df_std, df_mean + df_std, 
                   alpha = 0.5, **kwargs
                  )
    return df_mean , df_std

def plot_it(a, ds, variable, direction='up' , color = None, next_color=None, cloud_base = True, cloud_base_trans = True, cloud_top = True, **kwargs):
    ...
    #     df_mean = df.down
    #     df_std = df.down_std
    # elif direction == 'up_ground2cb':
    #     df_mean = df.
    # else:
    #     raise KeyError(f'{direction} is not an option for direction')
    ...
    if cloud_base:
        ...
        if cloud_base_trans:
            out = plot_avg(a, ds.cloud_base_transit_altitude, direction = direction, color = 'm', 
                     # show_std = False,
                      ls = '--', 
                      lw = 0.8
                     )
...
def plot_set(ds, direction = 'up', plot_cb = True, plot_cb_trans = True, plot_ct = True,  ax = None, alt = False ):
    ...
    out = plot_it(a_tmp, ds, ds.temperature, direction=direction, cloud_base = plot_cb, cloud_base_trans = plot_cb_trans, cloud_top = plot_ct)
    at = a_tmp.twiny()
    a_tmp.at = at
    ...
    at.set_xlabel('$\\theta$')
    at.legend(handles = [g, gtp, gtep], loc = 9, fontsize = 'x-small')
    # rh
    plot_it(a_rh, ds, ds.relative_humidity, direction=direction, cloud_base = plot_cb, cloud_base_trans = plot_cb_trans, cloud_top = plot_ct)
    a_rh.set_xlabel('RH (%)')

    #lwp & precip
    out = plot_it(a_lwp, ds, ds.remote_mwr_liquid_water_path, direction=direction, cloud_base= plot_cb, cloud_base_trans = plot_cb_trans, cloud_top = plot_ct)
    glwp = out['g']
    glwp.set_label('lwp')
    at = a_lwp.twiny()
    a_lwp.at = at
    ...
    at.legend(handles = [glwp, gpc], loc = 9)

    # nc
    out = plot_it(a_nc, ds, ds.pops_particle_number_concentration, direction=direction, cloud_base = plot_cb, cloud_base_trans = plot_cb_trans, cloud_top = plot_ct)
    gp = out['g']
    gp.set_label('POPS')
    ...
    # mean diameter
    plot_it(a_md, ds, ds.pops_particle_mean_diameter, direction=direction, cloud_base = plot_cb, cloud_base_trans = plot_cb_trans, cloud_top = plot_ct)
...
def get_plot_lim(ax,lower_alt_lim = 20):
    ...
    if len(mins) == 0:
        ...
        lims = [None if np.isnan(l) else l for l in lims]
        lims = [None if np.isinf(l) else l for l in lims]
    return lims

def plot_quicklook(ds, ground2cloudbase = False, plot_cb = True, plot_cb_trans = True, plot_ct = True, 
                   save = True, output_path = None, lower_alt_lim = None, 
                   overwrite = False):
    """
    ...
    plot_set(ds, direction = 'up' + addon, ax = aa[0], plot_cb = plot_cb, plot_cb_trans = plot_cb_trans, plot_ct = plot_ct)
    plot_set(ds,direction='down' + addon, ax = aa[1], plot_cb = plot_cb, plot_cb_trans = plot_cb_trans, plot_ct = plot_ct)
