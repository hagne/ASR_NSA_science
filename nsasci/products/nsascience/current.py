...
def set_altitude_column(imet, alt_source, log = {}):
    logentry = {'success': True}
    logentry['problems'] = []
    log['set_altitude_column'] = logentry
    if 'baro' in alt_source:
        ...
        # return False
        txt = 'bad altitude reading (bar and gps)'
        warnings.warn(txt)
        logentry['problems'].append(txt)
        logentry['success'] = False
        return False
        # raise ValueError('data quality of altitude measurment assigned as bad -> no product')
    else:
...
def load_met_files(start_time, end_time, folders):
    ...
    for idx, path in met_file_df_match.iterrows():
        ...
        precip = ds.pwd_precip_rate_mean_1min.to_dataframe()
        precip_resamp = precip.resample('1s').interpolate()
        tl2.append(precip_resamp)


    press_df = pd.concat(tl, sort=True)
    precip_df = pd.concat(tl2, sort = True)
    
    press_df = press_df.truncate(start_time, end_time)
    precip_df = precip_df.truncate(start_time, end_time)
    out = {}
    out['press'] = press_df * 10
    out['precip'] = precip_df
    return out
...
def add_cloud_base_distance_and_transit(dst, ceilometer_folder = '/mnt/telg/data/arm_data/OLI/ceilometer/', log = {}):
    ...
    logentry = {'success': True}
    logentry['problems'] = []
    log['add_cloud_base_distance_and_transit'] = logentry
...
#### file missing
    ...
    if tdmin > pd.Timedelta(1, 'd'):
        ...
        tmp = dst.altitude.copy()
        tmp[:] =  -9999
                
        first_cloud_base = tmp.copy()
        trans_df = tmp.copy()
        dist2cb = tmp.copy()
    else:
        ...
        while True:
            ...
            while off_center and its < its_max:
                ...
                try:
                    ...
                    txt = 'issue with linear gregression when looking for cloud pass ... try next'
                    warnings.warn(txt)
                    logentry['problems'].append(txt)
                    valid = False
                    break
        ...
        cb.First_cloud_base.index.name = 'datetime'
        trans_df = pd.DataFrame(trans_df, index = dst_alt.index)
        trans_df = trans_df.trans.astype(float)
        dist2cb.index.name = 'datetime'
        first_cloud_base = cb.First_cloud_base
### add to xarray dataset

    dst['remote_ceilometer_cloud_base_altitude'] = first_cloud_base
    
    dst['cloud_base_transit'] = trans_df
    dst.cloud_base_transit.attrs['info'] = f'1 - acent, -1 - decent, 0 - unclear (r^2 < {direction_min_r2}); -9999 required file was missing'
...
def add_mwr_products(dst, path = '/mnt/telg/data/arm_data/OLI/max_mwr/MWR/', log = {}):
    """
...
### find the matching files
    ...
    ### open datasets and merge relevant data
        ...
        lwp = lwp.astype(float)

### add to nsascience dataset
    dst['remote_mwr_liquid_water_path'] = lwp
...
def process(ipmatchrow, folders, test = False,raise_error = True, path2product_file = None, log = {}):
    # imet
    ####
    ...
    if 'sn00' in fn_pops.name:
        fn_pops_old = fn_pops
        fn_pops = fn_pops.parent.joinpath(fn_pops.name.replace('sn00', f'sn{ipmatchrow.popssn}'))
        warnings.warn(f'replaced sn00 with sn{ipmatchrow.popssn}\n{fn_pops_old} -> \n{fn_pops}')
...
# standardize time
    ...
    dstbs.base_time.attrs['string'] = pd.to_datetime(dstbs.time.values[0]).__str__() + ' 0:00'
...
def make_product(version = '',
    ...
    for idx, ipmatchrow in imet_pops_match.iterrows():
        logentree = {}
        logentree['ipmatchrow'] = ipmatchrow
        # generate tbs name
    ...
    #     break
        ...
        ds = process(ipmatchrow, folders, path2product_file = path2product_file, log = logentree)
        if not ds:
        ...
        # try:
        #     ds = process(ipmatchrow, folders, path2product_file = path2product_file, log = log)
        #     if not ds:
        #         txt = 'ds is not (execution of process(ipmatchrow) failed)'
